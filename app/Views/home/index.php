...
<?php if (!empty($makanan)): ?>
    <?php foreach ($makanan as $item): ?>
        <?php if (session()->get('role') === 'pembeli' && $item['status'] === 'Tersedia'): ?>
            <!--
                PENTING: modal ini SENGAJA diletakkan di luar .card-menu (bukan nested di dalam card).
                Alasan: .card-menu punya CSS `transform` (efek hover naik) + `overflow: hidden`.
                Ancestor yang punya `transform` aktif jadi containing block baru untuk descendant
                `position: fixed` (termasuk .modal Bootstrap). Karena mouse pasti sedang hover di atas
                tombol "Pesan" saat diklik, transform ikut aktif, sehingga modal sempat "terjebak" /
                terklip di dalam kotak card kecil yang overflow:hidden selama 1 frame, lalu "meloncat"
                balik ke posisi normal begitu hover/focus berubah. Itulah penyebab efek kedap-kedip.
                Jangan kembalikan modal ke dalam .card-menu.
            -->
